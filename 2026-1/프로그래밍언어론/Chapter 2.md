...
프로그래밍 언어 관점에서 컴퓨터는 프로그램을 저장하고 실행할 수 있는 알고리즘과 자료 구조의 집합이다.
다음 주요 구성요소를 가진다.
...
위 구성 요소를 갖춘 컴퓨터는 아래의 Machine Cycle에 따라서 프로그램을 실행한다.

![](https://velog.velcdn.com/images/kmwook/post/9c62c724-22f8-4dd4-b2e4-a621ab864b80/image.png)

정확하게 정의된 알고리즘 또는 자료구조는 하드웨어로 구현될 수 있다는 이론적 기반 아래에 **Firmware Computer**가 개발되었다.
Firmware Computer는 하드웨어와 이를 제어하기 위한 **마이크로 프로그램(microprogram; firmware)** 으로 구성되며, 이 마이크로 프로그램을 소프트웨어적으로 흉내내는 것을 에뮬레이션(Emulation)이라고 한다.

고수준(High-level) 언어로 작성된 프로그램을 기계어(machine language) 설계에 관계 없이 컴퓨터에서 실행시키기 위해서는 두 가지 방법이 존재한다.
...
전형적인 언어 구현 구조는 아래와 같다.

![](https://velog.velcdn.com/images/kmwook/post/5c4c44d5-4783-47ed-96bf-17649a5a5e72/image.png)

프로그래밍 언어를 구현할 때, 프로그램 실행 시에 사용하는 알고리즘과 자료구조를 Computer라고 정의한다.

언어 구현에 의하여 정의된 컴퓨터를 **virtual computer**라고 하는데, virtual computer의 기계어는 언어 번역기에 의해 실행 가능한 프로그램이다.

즉, **구현된 프로그래밍 언어는 그 자체로 virtual computer로 볼 수 있다.**
